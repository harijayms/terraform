...
func TestAccAzureRMVirtualMachine_encryptedWindows(t *testing.T) {
	var vm compute.VirtualMachine
	suffix := fmt.Sprint(acctest.RandInt())
	config := strings.Replace(testAccAzureRMVirtualMachine_encryptedWindows, "[suffix]", suffix, 0)
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		CheckDestroy: testCheckAzureRMVirtualMachineDestroy,
...
var testAccAzureRMVirtualMachine_encryptedWindows = `
variable "suffix" { default = "[suffix]" }
